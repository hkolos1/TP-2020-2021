...
public:
  ...
  const char *DajImeDanaUSedmici() const {
    return imena_dani[static_cast<int>(this->DanUSedmici()) - 1];
  }
  Datum &operator++();
  Datum operator++(int);
  Datum &operator--();
...
int Datum::MjesecToInt(Datum::Mjeseci m) {
  return std::abs(
      (static_cast<int>(m) - static_cast<int>(Mjeseci::Januar)) % 12 + 1);
}
...
Datum::Dani Datum::DanUSedmici() const {
  ...
  return _reminder == 0 ? Dani::Nedjelja
                        : static_cast<Datum::Dani>(_reminder +
                                                   int(Datum::Ponedjeljak) - 1);
}
...
Datum &Datum::operator--() {
  if (1 == this->dan) {
    ...
    --this->dan;
  try {
    validDate(this->dan, this->mjesec, this->godina);
  } catch (...) {
    ++*this;
    throw;
  }
...
inline Datum operator+(int days, const Datum &date) { return date + days; }
...
Datum::operator std::string() const {
  return std::to_string(this->DajDan()) + '.' + this->DajImeMjeseca() + ' ' +
         std::to_string(this->DajGodinu()) + ".(" + this->DajImeDanaUSedmici() +
         ")";
}
...
const char *Datum::imena_mjeseci[12] = {
    "Januar", "Februar", "Mart",      "April",   "Maj",      "Juni",
    "Juli",   "August",  "Septembar", "Oktobar", "Novembar", "Decembar"};
const char *Datum::imena_dani[7] = {"Ponedjeljak", "Utorak", "Srijeda",
                                    "Cetvrtak",    "Petak",  "Subota",
                                    "Nedjelja"};
const int Datum::broj_dana[12] = {31, 28, 31, 30, 31, 30,
                                  31, 31, 30, 31, 30, 31};
int main() { return 0; }