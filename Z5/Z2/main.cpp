...
private:
  ...
  void static provjeraParametara(int num, int M = 1, double p = 1, double q = 1,
                                 bool ntype = false) {
    if (num <= 0)
...
public:
  ...
  FourierovRed(double T, std::initializer_list<double> k_a,
               std::initializer_list<double> k_b);
  FourierovRed(int N, double T, std::function<double(int)> funk_a,
               std::function<double(int)> funk_b);
...
FourierovRed::~FourierovRed() { this->IsprazniMemoriju(); }
...
FourierovRed::FourierovRed(double t, std::initializer_list<double> k_a,
                           std::initializer_list<double> k_b)
    : T(t), a(k_a) {
...
FourierovRed::FourierovRed(int N, double T, std::function<double(int)> funk_a,
                           std::function<double(int)> funk_b)
    : N(N), T(T) {
  provjeraParametara(N);
...
FourierovRed::FourierovRed(int N, double p, double q,
                           std::function<double(double)> f, int M)
    : N(N), T(q - p) {
  provjeraParametara(N, M, p, q, true);
  double konstanta((f(p) + f(q)) / M);
  auto gen_function = [=](int k,
                          std::function<double(double)> nizFunkcija) -> double {
    double sum(0);
    for (int i(1); i < M; ++i)
      sum += f(p + (i * T / M)) *
             nizFunkcija((2 * k * PI * (p / T + double(i) / M)));
...
FourierovRed::FourierovRed(const FourierovRed &red)
    : N(red.N), T(red.T), a(red.a), b(red.b) {}
